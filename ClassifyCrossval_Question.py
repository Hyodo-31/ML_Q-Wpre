...
# newArray = numpy.array(oldArray)というの風に使うと，oldArrayが，numpy形式の配列に変換されて，newArrayに代入されます．
...
for q in range(10):	#10回繰り返す
	...
	sel_count = min(len(labels2), len(newcounter))
	idx_yes = random.sample(range(len(labels2)), sel_count)
	labels2_sel = [labels2[i] for i in idx_yes]
	features2_sel = [features2[i] for i in idx_yes]

	for _ in range(sel_count):	#labels4とfeatures4からランダムでありの数とおなじだけなしのデータを取り出しnewlabelsとnewfeaturesに格納
	...
	for t in labels2_sel:	#なしのデータが入っているnew～にありのデータを付け加える
		newlabels.append(t)
	for u in features2_sel:
