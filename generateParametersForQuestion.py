...
readers = []
fread1 = open('inputdataAll1.csv', 'r')
readers.append(csv.reader(fread1))
fread2 = open('inputdataAll2.csv', 'r')
		...
		for question in data[user].keys():
			...
			for i in range(len(params)-1): # 今の行とその次の行の比較によって求めるパラメータがあるので，outofrangeになるのを防ぐためにlen-1にする
				...
				if params[i+1]['x']-params[i]['x'] < 0:
					...
					xDirection = 0
				if (xDirection == 1 and lastXDirection == -1) or (xDirection == -1 and lastXDirection == 1):
					xUTurnCount += 1
				...
				if params[i+1]['y']-params[i]['y'] < 0:
					...
					yDirection = 0
				if (yDirection == 1 and lastYDirection == -1) or (yDirection == -1 and lastYDirection == 1):
					yUTurnCount += 1
